package com.kh.chapter01_abstraction.run;

import com.kh.chapter01_abstraction.model.vo.Student;

public class Run {
	/*
	 * 1. 객체지향 언어 : "객체"를 "지향"하는 언어로 객체중심으로 돌아가는 언어
	 * 
	 * 2. 객체란 ? 사전적 의미로는 현실세계에 독립적으로 존재하는 모든것을 의미
	 * 
	 * 3. 객체지향 프로그래밍 : 현실세계의 객체간의 상호작용(행위)를 통해 가상세계(코드)로 구현하는 과정
	 * 
	 ...
	 * 
	 * 5. 추상화 과정
	 *    1) 내가 구현하고자 하는 프로그램에서 필요한 객체들을 생각해볼것 ex) 학생관리 프로그램- > '학생 객체' : 홍길동 학생.. 민경민학생..
	 ...
	 *    		2) 공통적인 속성 및 기능들(이름, 나이 , 성별, 주소 , 전화번호, 키, 달리기 속도, 학년, 반, 국어점수, 영어점수 ,몸무게, 자기, 먹기, 생각하기)
	 *    		3) 만약 학생 "인적사항"을 관리하는 프로그램이라면? 이름, 나이, 성별, 주소, 전화번호, 학년, 반 만 추려낸다
	 * 				만약 학생 "성적"을 관리하는 프로그램이라면? 이름, 학년, 반, 국어점수, 수학점수 만 추려낸다
	 * 				만약 학생 "건강"을 관리하는 프로그램이라면? 이름, 키, 학년, 반, 몸무게,.. 만 추려낸다
	 ...
	 * 				홍길동이라는 학생객체를 만들기 위해서 => String name1 = "홍길동", int age1 = 20; double height1 = 180;
	 * 				민경민이라는 학생객체를 만들기 위해서 => String name2 = "민경민", int age2 = 30; double height2 = 190;
	 * 											=> String nameN = ...;
	 * 				=> 관리하고자 하는 학생들이 늘어나면 늘어날수록 생성해야 할 갯수도 엄청나게 증가하게됨. 즉, 관리하기가 힘듦.
	 * 
	 ...
	 *    				그리고 새로운 학생이 들어오게 되는경우 배열의 특성상 한번 크기를 지정하게되면 변경하는게 불가능하므로, 새로운 
	...
	public static void main(String[] args) {
		...
		hong.name = "홍길동";
		hong.age = 20;
		hong.height = 166.7;//필드 접근하고자 한다면 . 을 통해 가져올 수 있음(단, 접근제한자가 public인 경우)
		...
		System.out.println(hong.name+"님의 나이는 "+hong.age+"살이고 키는 "+hong.height+"cm 입니다.");
		System.out.printf("%s님의 나이는 %d살이고 키는 %.1fcm 입니다.", hong.name, hong.age, hong.height);
		/*
		 * 필드 public으로 작성시
		 *  => 직접적으로 접근해서 값 대입 및 조회 할 경우 보안 문제가 생길 수 있다.
		 *  => 외부에서 함부로 값을 변질시키거나 조회권한이 없는 사람이 값을 가져올 수 있는 문제 발생
		 *  >>> 캡슐화로 해결

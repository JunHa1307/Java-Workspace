...
public class ListRun {
	...
	public static void main(String[] args) {
		 ...
		 * ==컬렉션
		 * => 다량의 데이터들을 관리하고자 할 때 배열가지도 충분히 사용했었음, 단 배열의 단점들을 보완한 것이 컬랙션
		 * 
		 * 배열과 컬렉션의 차이점
		 * - 배열의 다점 
		 * 1. 배열을 쓰고자 할 때 먼저 크기를 지정해야함.
		 * 		=>  한번 지정된 크기는 변경이 불가
		 * 			새로운 값을 추가하고자할 때 크기가 오버될 경우 새로운 크기의 배열을 만들고 기존의 내용들을 복사해주는
		 * 			코드를 직접 구현해야함.
		 ...
		 * 3. 기본적으로 여러 타입의 데이터들을 저장 할 수 있음(단, 제네릭 설정을 통해서 한 타입의 데이터들만 들어올수도 있음)
		 * 
		 ...
		 * 2. 컬랙션에 저장된 객체를 꺼내서 사용할 때 매번 형변환 하는 절차를 없애기 위해서
		 */
		//1. add(E e) : 해당 리스트의 끝에 전달된 e 를 추가시며주는 메소드
		...
		System.out.println(list);
		/*
		 * 2. add(int index,E e) 리스트에 전달되는 index값 위치에 e를 추가시켜주는 메서드
		 */
		list.add(1, new Music("바다의 왕자", "박명수"));
		// 중간에 값 추가시 알아서 기존의 값들을 뒤로 땡겨주는 작업이 내부적으로진행됨
		...
		// 4. remove(int index, E e) : 리스트에서 해당 인덱스 위치엥 있는 값을 삭제시켜주는 메서드
		list.remove(1);
		System.out.println(list);
		
		// 5. size() : 리스트에 담겨있는 데이터의 수를 반환해주는 메서드
		System.out.println("리스트에 담긴 데이터 수 : " + list.size());
		System.out.println("리시트의 마지막 인덱스 : " + (list.size() -1));
		
		// 6. get(int index) : E => 리시트의 해당 index위치의 데이터를 반환해주는 메서드
		// 제네릭 설정 전
		// Music m = (Music) list.get(0);// 다형성 => 강제 형변환
		...
		System.out.println("========================================================");
		// for-each문 : 향상된 반복문
		// for (값을 담아줄 변수  :  순차적으로 접근할 배열, 컬랙션)
		for(Music o : list) {
